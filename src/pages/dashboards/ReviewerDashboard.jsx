...
const ReviewerDashboard = () => {
  ...
  const [applyRedactionBlur, setApplyRedactionBlur] = useState(true);
  const [applyBylineMask, setApplyBylineMask] = useState(true);
  const [bylineHeight, setBylineHeight] = useState(130);
  const [applyTableBioMask, setApplyTableBioMask] = useState(true);
  const [tableBioTop, setTableBioTop] = useState(24);
  const [tableBioHeight, setTableBioHeight] = useState(240);
  const [redactionMode, setRedactionMode] = useState('blackout'); // 'blackout' | 'blank' | 'blur'
  ...
  return (
    <div className="space-y-8">
          ...
          filteredReviews.map(rev => {
            ...
            return (
              ...
              >
                ...
                {/* Submitted Review Comments Summary if Completed */}
                {rev.review_comments && (
      ...
      {/* 5. Interactive Double-Blind Manuscript Viewer with Author Redaction Overlay */}
      {viewingDoc && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100] p-4 sm:p-6 animate-fadeIn">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-5xl h-[90vh] flex flex-col relative overflow-hidden border border-gray-200">
            ...
            {/* Modal Header with Redaction Notice */}
              ...
              <div className="flex flex-wrap items-center gap-2 self-end sm:self-center">
                ...
                <button
                  onClick={() => setActiveLatexArticle({
                    ...
                    latex: viewingDoc.latex || ''
                  })}
                  className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all border bg-emerald-500/20 border-emerald-400 text-emerald-300 hover:bg-emerald-500/30 flex items-center gap-1.5"
                >
                  <FaSquareRootAlt /> LaTeX Source
                </button>
                <button 
            ...
            {/* Document Body - Clean Physical PDF Viewer (Physically Redacted by PHP Engine) */}
            <div className="flex-1 bg-gray-900 overflow-hidden relative flex flex-col">
              <div className="flex-1 w-full h-full relative overflow-hidden bg-gray-100">
                <object
                  data={
                    viewingDoc.url && (viewingDoc.url.toLowerCase().endsWith('.doc') || viewingDoc.url.toLowerCase().endsWith('.docx'))
                ...
                >
                  <iframe 
                    src={
                      viewingDoc.url && (viewingDoc.url.toLowerCase().endsWith('.doc') || viewingDoc.url.toLowerCase().endsWith('.docx'))
                        ? `https://docs.google.com/viewer?url=${encodeURIComponent(viewingDoc.url)}&embedded=true`
                        : (viewingDoc.streamUrl || viewingDoc.url)
                    }
                    title="Double-Blind Manuscript Viewer" 
                    className="w-full h-full border-0 bg-white" 
                  >
                    <div className="p-8 text-center text-gray-600">
                      <p className="font-bold">PDF preview is ready.</p>
                      <a href={viewingDoc.url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block px-4 py-2 bg-gray-900 text-white rounded-lg text-xs font-bold">
                        Open PDF in New Tab ↗
                      </a>
                    </div>
                  </iframe>
              ...
              {/* Viewer Footer Bar with fallback link */}
              <div className="px-5 py-2 bg-gray-900 text-gray-300 text-xs flex flex-wrap justify-between items-center gap-2 shrink-0 border-t border-gray-800">
                <div className="flex items-center gap-2">
                  <FaFilePdf className="text-amber-400" />
                  <span>
                    Manuscript preview active. Having trouble viewing?{' '}
                    <a 
                      href={viewingDoc.url} 
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="text-amber-400 font-bold underline hover:text-amber-300 ml-1"
                    >
                      Open full PDF directly ↗
                    </a>
                  </span>
                </div>
                <span className="font-mono text-[10px] text-gray-400 truncate max-w-sm">
                  {viewingDoc.url.split('/').pop()}
                </span>
              </div>
      ...
      {/* 6. Comprehensive Review Evaluation & Scoring Modal (Supports Active Commenting for Pending Tasks) */}
      {activeReviewModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-xl max-w-3xl w-full p-6 sm:p-8 shadow-lg border border-gray-200 my-8 animate-scaleUp max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSubmitReview(false); }} className="space-y-6 pt-5">
              ...
              {/* Review Comments to Author & Editor */}
              <div>
                ...
                <textarea
                  ...
                  placeholder="Provide structured feedback covering:
1. Summary of scholarly contribution
2. Key strengths of the paper
3. Conceptual & theoretical areas for improvement
4. Specific paragraph/line recommendations..."
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Private remarks to the editor regarding suitability for special issues, ethical compliance, or confidential reservations..."
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 space-y-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-gray-700">
                  Optional: Upload Annotated Manuscript / Review Report (.pdf, .doc, .docx)
                </label>
                <input
                  type="file"
                  accept=".pdf,.doc,.docx"
                  onChange={(e) => setFormData({ ...formData, reviewFile: e.target.files[0] })}
                  className="w-full text-xs text-gray-600 file:mr-3 file:py-2 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-gray-200 file:text-gray-900 cursor-pointer"
                />
                {activeReviewModal.review_doc_url && (
                  <p className="text-xs text-blue-700 mt-1">
                    Current attachment: <a href={resolveFileUrl(activeReviewModal.review_doc_url)} target="_blank" rel="noopener noreferrer" className="underline font-bold">Download Current Review Document</a>
                  </p>
                )}
              </div>
      ...
      {/* 7. Interactive LaTeX & Equation Source Editor with Automatic Double-Blind Anonymizer */}
      <LatexEditorModal
